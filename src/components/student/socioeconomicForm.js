...
import ConsultaDatosSocioeconomicos from '../consutaDatosPersonales/consultaDatosSocioeconomicos';
...
const SocioeconomicForm = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${BASE_URL}/user/datosresidencia/${id}?token=${token}`)
            .then(response => {
                const {
                ...
                if (isEditable) {
                    try {
                        const response = await axios.put(
                            `${BASE_URL}/user/datosresidencia/${id}?token=${token}`,
    ...
    return (
        <div className='container-form-sgdar'>
            <h1 className='title-form'>Ingresa tus datos socioeconomicos</h1>
