...
const AcademicForm = ({userId}) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${BASE_URL}/user/datosinstitucionales/${userId}?token=${token}`)
            .then(response => {
                const {
    ...
    const saveAcademicForm = async () => {
        ...
        if (noEdit) {
            sweetAlert(
                WARNING,
                'Atención',
                'Para poder guardar debe habilitar el fomulario, haciendo clic en el botón editar.'
            );
        } else {
            if (valuesValidate.length === 8) {
                ...
                try {
                    const response = await axios.put(
                        `${BASE_URL}/admin/users/modificarusuario/datosinstitucionales/${userId}?token=${token}`,
                        requestAcademic
                    );
