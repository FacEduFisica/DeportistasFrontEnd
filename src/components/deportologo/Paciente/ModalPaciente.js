...
import { MESSAGE_NOFOUND_CEDULA, MESSAGE_NOFOUND_STUDENT } from '../../FormConstants/constants';
import { WARNING, BASE_URL, ERROR } from '../../../constantGlobal';

import './modalPaciente.css';

const ModalPaciente = ({ setModalpaciente }) => {
            ...
            <div className='container-modal-paciente' style={{ width: 750 }}>
                ...
                {viewInfo ? (
                    <div className='container-info-paciente'>
                        ...
                        <div className='infoPaciente'>
                            <span className='text-info-paciente'>{`Direccion: ${infoPaciente.direccion}`}</span>
                            <span className='text-info-paciente'>{`Teléfono: ${infoPaciente.telefono}`}</span>
                        </div>
                        <div className='infoPaciente'>
                            <span className='text-info-paciente'>{`Estado civil: ${infoPaciente.estadoCivil}`}</span>
                        </div>
                    </div>
                ) : cargando ? (
